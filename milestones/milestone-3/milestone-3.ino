...
void dfs(){
    ...
    trace.push(currVal);
    Serial.println("");
    Serial.print("value from frontier to trace: ");
    ...
    if ((next == 2 || next == 1) && !frontier.isEmpty()){
        ...
        Serial.println("doing backtrack now");
        trace.pop();
        backTrack = true;
...
void loop() {
  ...
  if (!backTrack) dfs();
  if (backTrack){
      Serial.print("trace in backtrack: ");
      Serial.println(trace.peek());
